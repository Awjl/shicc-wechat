<template>
  <div class="NoticeList">
    ...
    <div class="NoticeListBox" v-if="show">
      <div class="NoticeListBox-title">{{noticeContent.title}}</div>
      <p class="NoticeListBox-time">{{noticeContent.createTime}}</p>
      <div class="NoticeListBoxCon">{{noticeContent.content}}</div>
      <div class="NoticeListBox-btn" @click="hideBox">确定</div>
    </div>
    <div class="integralListbox" v-if="noticeList.length == 0">- 暂无通知记录 -</div>
  </div>
</template>
